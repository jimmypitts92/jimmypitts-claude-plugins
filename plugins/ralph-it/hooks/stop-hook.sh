...
if [[ -z "$PROMPT_FILE" ]]; then
  echo "⚠️  Ralph loop: State file missing prompt_file field" >&2
  echo "   File: $RALPH_STATE_FILE" >&2
  echo "" >&2
...
if [[ ! -f "$PROMPT_FILE" ]]; then
  echo "⚠️  Ralph loop: Prompt file not found" >&2
...
if [[ ! -r "$PROMPT_FILE" ]]; then
  echo "⚠️  Ralph loop: Prompt file is not readable" >&2
...
if [[ -z "$PROMPT_TEXT" ]]; then
  echo "⚠️  Ralph loop: Prompt file is empty" >&2
  echo "   File: $PROMPT_FILE" >&2
  echo "" >&2
  echo "   Ralph loop is stopping." >&2
  rm "$RALPH_STATE_FILE"
